...
Feature: Reading the features on main
  As the driver of a big project
  I see every feature on main and read any one of them
  so that I can tell what is done and what is missing, and decide what gets built next and how to slice it.

  Scenario: The features on main are listed with their title, id and domain
    Given main holds the feature "Paying with a saved card" with the id "pay-with-a-saved-card" in the domain "payments"
  ...
  Scenario: A file that is not a readable feature is listed as broken
    Given main holds "payments/refunds.feature", which is not a readable feature
    When I open the portal
    Then I see "refunds.feature" listed as broken

  Scenario: Main holds no features
    Given main holds no features
    When I open the portal
    Then I am told main has no features yet

  Rule: What is declared for later shows as backlog

    Scenario: A feature tagged backlog is listed as backlog
      Given main holds the feature "Earning points" tagged "@backlog"
      When I open the portal
